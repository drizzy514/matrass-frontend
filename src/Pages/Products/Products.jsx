...
function Products() {
  ...
  // hooks
  const { message, setBody, method, setMethod } = useFetch("/addproduct");
  ...
  const handleSubmit = (e) => {
    ...
    if (inputs.length && url.length) {
      setBody({
        ...
        garanty: parseInt(inputs[4].value),
        kg: inputs[5].value,
        size: parseInt(inputs[6].value),
        place: parseInt(inputs[7].value),
        action: parseInt(inputs[8].value) ? parseInt(inputs[8].value) : null,
        description: inputs[10].value,
        status_active: inputs[12].checked,
      });
    }
    setModalLoading(true);
  };
  ...
  return (
    <div>
      ...
      {products && products.length && products[0].products_id ? (
        <table>
          <thead>
            <tr>

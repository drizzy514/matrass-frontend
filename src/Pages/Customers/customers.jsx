...
function Customers() {
  ...
  const { message, setBody, method, setMethod } = useFetch("/makeancustomers");
  ...
  return (
    <div>
      ...
      {customers && customers.length && customers[0].customers_id ? (
        <table>
          <thead>
            <tr>
              <th>ID</th>
          ...
          <tbody>
            {customers.map((c, i) => {
              const date = c.customers_date ? c.customers_date.split(/[.T\s/]/) : "";
              return (
                <tr key={c.customers_id} className="table_tr">
                  <td>{i + 1}</td>
                  <td>
                    {date[1]}-<span className="date">{date[0]}</span>
                  </td>
                  <td>{c.customers_phone}</td>
                  <td>
                    <div className="customers_checkbox_wrapper">
                      <label className="checkbox-container customers_checkbox-container">
                        <input
                          defaultChecked={c.customers_phone}
                          required
